...
class ThytanicController:
    ...
    def __init__(self):
        ...
        self.k_alpha = 20# NEED TO BE FINE-TUNED
        #self.k_beta = -15 # NEED TO BE FINE-TUNED
        self.wheel_radius = 2.1
        self.axle_length = 9.5 
        self.min_distance = 2 # NEED TO BE FINE-TUNED
        self.max_distance = 10 # NEED TO BE FINE-TUNED
    ...
    def read_wheel_speed(self):
        ...
        """
        aw(self.robot_node.wait_for_variables({'motor.left.speed',
                                        'motor.right.speed'}))
        speed_values = [self.robot_node.v.motor.left.speed *  self.conversion_factor/self.mm_per_pixel,
                        self.robot_node.v.motor.right.speed*  self.conversion_factor/self.mm_per_pixel]
    ...
    def update_robot_state(self):
        ...
        if self.robot_state == ThytanicState.GLOBAL_MOVEMENT and not is_obstacle_detected:
            self.control_robot()
    ...
    def astolfi_control(self, state_est):
        ...
        if (alpha > np.pi): alpha= (alpha + np.pi) % (2 * np.pi) - np.pi
        if (alpha < - np.pi): alpha= (alpha + 2*np.pi) % (2 * np.pi)
