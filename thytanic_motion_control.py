...
- ThytanicController: Control the Thymio.
"""
...
class ObstacleDirection(Enum):
    """Specify whether an obstacle is detected on the left or right."""
...
class ThytanicController:
    ...
    def rotate_robot(self, direction, rotation_speed):
        ...
        Arguments:
        - direction: Direction to rotate (ObstacleDirection.LEFT_SIDE or RIGHT_SIDE).
        - rotation_speed: Speed of the rotation for both wheels.
        """
        if direction == ObstacleDirection.LEFT_SIDE:
            self.set_wheel_speed(-rotation_speed, rotation_speed)
        elif direction == ObstacleDirection.RIGHT_SIDE:
            self.set_wheel_speed(rotation_speed, -rotation_speed)
    ...
    def maneuver_around_obstacle(self, sensor_readings):
        ...
        if any(value > self.detection_threshold for value in sensor_readings[0:2]):
            self.rotate_robot(ObstacleDirection.RIGHT_SIDE, self.avoidance_turn_speed)
            return ObstacleDirection.LEFT_SIDE

        # Turn left if the obstacle is on the right side
        elif any(value > self.detection_threshold for value in sensor_readings[2:5]):
            self.rotate_robot(ObstacleDirection.LEFT_SIDE, self.avoidance_turn_speed)
            return ObstacleDirection.RIGHT_SIDE
    ...
    def update_robot_state(self):
        ...
        if self.robot_state == State.STOP:
            self.set_wheel_speed(0, 0)
            return

        if self.robot_state == State.GLOBAL_MOVEMENT and is_obstacle_detected:
            self.robot_state = State.AVOIDING_OBSTACLE
            self.obstacle_side = self.maneuver_around_obstacle(sensor_data)

        elif self.robot_state == State.AVOIDING_OBSTACLE and not is_obstacle_detected:
            self.robot_state = State.GLOBAL_MOVEMENT
            self.set_wheel_speed(self.normal_speed, self.normal_speed)

        elif self.robot_state == State.AVOIDING_OBSTACLE and is_obstacle_detected:
